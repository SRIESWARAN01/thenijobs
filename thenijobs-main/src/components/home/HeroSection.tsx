...
export default function HeroSection() {
  ...
  const statsList = [
    ...
    { value: totalUsers.toLocaleString('en-IN'), label: 'Job Seekers' },
  ];
        ...
        <div className="relative">
          <div className="rounded-[2rem] border border-slate-200 bg-white p-3 shadow-[0_24px_70px_rgba(15,23,42,0.14)]">
            ...
            <div className="grid gap-3 p-3 sm:grid-cols-3">
              <Link href="/jobs" className="flex items-center justify-center gap-2 rounded-xl bg-teal-50 px-3 py-3 text-xs font-black text-teal-800">
                <Phone size={15} />
                Jobs
              </Link>
              <Link href="/businesses" className="flex items-center justify-center gap-2 rounded-xl bg-emerald-50 px-3 py-3 text-xs font-black text-emerald-800">
                <MessageCircle size={15} />
                Business
              </Link>
              <Link href="/services" className="flex items-center justify-center gap-2 rounded-xl bg-amber-50 px-3 py-3 text-xs font-black text-amber-800">
                <Navigation size={15} />
                Services
              </Link>
            </div>
          ...
          <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="space-y-2">
              {updates.length > 0 ? updates.map((item, idx) => (
                ...
                <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 p-4 text-center">
                  <p className="text-sm font-black text-slate-900">No live updates yet</p>
                  <p className="mt-1 text-xs font-semibold text-slate-500">Admin activity and approvals will appear here.</p>
                </div>
              )}
            </div>
