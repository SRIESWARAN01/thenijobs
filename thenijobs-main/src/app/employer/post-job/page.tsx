...
import { createJobPosting } from '@/lib/firebase/platformActions';
...
export default function PostJobPage() {
  ...
  const planLimits = { ...getPlanLimits(plan), ...(dynamicPlanSettings?.[plan] || {}) };
  const canUseBoosts =
    planLimits.canUseFeaturedJobs &&
    planLimits.canUseUrgentJobs &&
    planLimits.canUsePremiumBadge;
  const { data: existingJobs } = useCollection<any>('jobs', [
    where('companyId', '==', companyId || '')
  ], { skip: !companyId });
  const countedJobs = existingJobs.filter((job) =>
    !['rejected', 'closed', 'archived'].includes(String(job.status || '').toLowerCase())
  );
  ...
  const handlePost = async () => {
    if (!companyId) {
      showToast('You must have a registered company profile to post a job.', 'warning');
      return;
    }

    if (!isUnlimitedLimit(planLimits.maxActiveJobs) && countedJobs.length >= planLimits.maxActiveJobs) {
      showToast(`${planLimits.name} plan allows ${planLimits.maxActiveJobs} active or pending job posting${planLimits.maxActiveJobs === 1 ? '' : 's'}. Please upgrade to post more.`, 'warning');
      return;
    }

    if ((form.isFeatured || form.isUrgent || form.isPremium) && !canUseBoosts) {
      showToast('Featured, urgent, and premium job boosts require Premium or Enterprise plan.', 'warning');
      return;
    }
    ...
    try {
      const jobData = {
        ...
        isPremium: canUseBoosts ? form.isPremium : false,
        isUrgent: canUseBoosts ? form.isUrgent : false,
        isFeatured: canUseBoosts ? form.isFeatured : false,
      ...
      showToast('Job posted successfully! It has been submitted for admin approval.', 'success');
      router.push('/employer/jobs');
    } catch (err) {
      console.error('Post job error:', err);
      showToast(err instanceof Error ? err.message : 'Failed to post job listing.', 'error');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-2.5 pt-4 border-t border-white/[0.06]">
              ...
              {[
                ...
                const activeClass = activeClassMap[color] || activeClassMap.cyan;
                return (
                  <div
                    key={key}
                    onClick={() => {
                      if (!canUseBoosts) {
                        showToast('Boost options require a plan with premium job features enabled.', 'warning');
                        return;
                      }
                    ...
                    className={`flex items-center justify-between p-3.5 rounded-xl border cursor-pointer transition-all
                      ...
                      ${canUseBoosts ? '' : 'opacity-60 cursor-not-allowed'}`}
                  >
                    <div>
