...
export function useCollection<T extends DocumentData>(
  ...
  // Serialise constraints so the effect re-runs when constraints actually change
  // (e.g. when filters update) but not on every render.
  const constraintsRef = useRef(queryConstraints);
  constraintsRef.current = queryConstraints;
  const constraintKey = JSON.stringify(
    queryConstraints.map((c) => `${c.type}:${JSON.stringify(c)}`)
  );
  ...
  }, [collectionName, options.skip, refreshKey, constraintKey]);
