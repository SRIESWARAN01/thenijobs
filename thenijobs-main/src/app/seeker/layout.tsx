...
import {
  ...
  Briefcase, Send, GraduationCap, Trophy
} from 'lucide-react';
...
const SEEKER_NAV = [
  ...
  { label: 'Rewards & Badges', tamilLabel: 'ரிவார்ட்ஸ்', icon: Trophy, href: '/seeker/rewards' },
...
export default function SeekerLayout({ children }: { children: React.ReactNode }) {
  ...
  // Derive profile completion from the user's actual data instead of a hardcoded value.
  const profileStrength = (() => {
    if (typeof user?.profileScore?.total === 'number') {
      return Math.max(0, Math.min(100, Math.round(user.profileScore.total)));
    }
    const checks = [
      !!user?.displayName,
      !!user?.photoURL,
          ...
          {collapsed ? (
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-emerald-400">🟢 {user?.preferences?.openToWork === false ? 'Not Looking' : 'Open to Work'}</p>
              </div>
      ...
      <div className={`flex-1 flex flex-col transition-all duration-300 ${collapsed ? 'lg:ml-[72px]' : 'lg:ml-[280px]'}`}>
        ...
        <header className="sticky top-0 z-30 h-16 bg-[#0a0a1a]/80 backdrop-blur-xl border-b border-white/[0.06] flex items-center px-4 lg:px-6 gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-emerald-500 to-cyan-500 flex items-center justify-center cursor-pointer hover:opacity-90 transition-opacity">
              <span className="text-white text-xs font-bold">{userInitials}</span>
            </div>
          </div>
        </header>
