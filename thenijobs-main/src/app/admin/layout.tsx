...
import {
  ...
  BarChart3, CreditCard, Megaphone, Shield, Settings, Bell, LogOut, ChevronLeft,
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const { user, loading: authLoading } = useRequireAuth(['admin', 'super_admin'], '/admin/login');
  ...
  const adminInitials = adminDisplayName.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2);
        ...
        <div className={`px-3 py-3 border-t border-white/[0.06] ${collapsed ? 'flex justify-center' : ''}`}>
          {collapsed ? (
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-white truncate">{adminDisplayName}</p>
