...
import {
  MapPin, Briefcase, Banknote, Clock, Users, BadgeCheck, BookmarkPlus, Share2, Zap, Building2, ChevronRight,
...
export default function JobDetailPageClient({ id }: { id: string }) {
  ...
  const resumes = useMemo(() => seekerProfile?.resumes || [], [seekerProfile?.resumes]);
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          setJob({
            ...
            deadline: d.deadline
              ? (typeof d.deadline === 'string' ? d.deadline : new Date(d.deadline.seconds * 1000).toLocaleDateString('en-IN'))
              : 'N/A',
    ...
    try {
      if (saved) {
        ...
        await addDoc(collection(db, 'savedJobs'), {
          ...
          skills: job.skills || [],
          deadline: job.deadline && job.deadline !== 'N/A' ? job.deadline : null,
  ...
  const handleApply = async () => {
    if (!uid || !job) {
      showToast('Please login as a seeker to apply.', 'warning');
      router.push('/login');
      return;
    }
