...
import {
  ...
  ShieldCheck, Smartphone, FileCheck, Award, ExternalLink,
...
export default function CompanyProfileClient({ company, jobs, reviews }: {
  ...
  const coverImage = company.coverUrl || company.coverImageUrl;
  const galleryImages = (company.galleryImages || company.gallery || []).filter(Boolean);
  ...
  const badges = [
    { key: 'mobileVerified', label: 'Mobile Verified', icon: Smartphone, color: 'emerald' },
    ...
    <main className="min-h-screen bg-[#0a0a1a]">
      ...
      <section className="pt-16">
        <div className="h-48 sm:h-64 relative overflow-hidden"
          style={{ background: 'linear-gradient(135deg, #1e0a5e 0%, #0a2a4e 50%, #0a1a3e 100%)' }}>
          {coverImage && (
            <img
              src={coverImage}
          ...
          {!coverImage && <div className="absolute inset-0 flex items-center justify-center text-8xl opacity-20">🌾</div>}
          {company.isPremium && (
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <div className="flex items-end gap-4 -mt-10 mb-6">
            {/* Logo */}
            <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl glass-card border-2 border-violet-500/30 flex items-center justify-center text-4xl sm:text-5xl shrink-0 shadow-2xl overflow-hidden">
              {company.logoUrl ? (
                  ...
                  <div className="space-y-3">
                    {jobs.map(job => (
                      <Link key={job.id} href={`/jobs/${job.id}`}
                        className="flex items-center justify-between p-4 rounded-xl bg-white/5 hover:bg-white/8 border border-white/10 hover:border-violet-500/30 transition-all group">
                        <div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                      {galleryImages.map((imageUrl: string, i: number) => (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="aspect-square rounded-xl bg-white/5 border border-white/10 overflow-hidden hover:border-violet-500/30 transition-all"
                        >
                          <img src={imageUrl} alt={`${company.name} gallery ${i + 1}`} className="h-full w-full object-cover" />
                        </a>
                      ))}
                    </div>
                  ) : (
                    <div className="rounded-xl bg-white/5 border border-white/10 p-8 text-center text-sm text-gray-500">
                  ...
                  {badges.map(({ key, label, icon: Icon, color }) => {
                    ...
                    return (
                      <div key={key} className={`flex items-center gap-3 p-2.5 rounded-xl text-sm
                        ${isVerified ? `bg-${color}-500/10 border border-${color}-500/20` : 'bg-white/5 border border-white/10 opacity-50'}`}>
                        <Icon size={14} className={isVerified ? `text-${color}-400` : 'text-gray-600'} />
