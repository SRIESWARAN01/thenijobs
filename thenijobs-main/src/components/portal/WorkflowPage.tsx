...
export interface WorkflowPageProps {
  ...
  showApplicationPipeline?: boolean;
}
...
function renderAction(action: WorkflowAction) {
  ...
  const classes = `inline-flex items-center justify-center gap-1.5 rounded-lg border px-3 py-2 text-xs font-semibold transition-all ${toneMap[action.tone || 'neutral']}`;
...
function parseTimestamps(timestamps?: Record<string, any>): Record<string, Date> | undefined {
    ...
    <div className="space-y-6 animate-fade-in-up">
          ...
          {primaryAction && (
            primaryAction.href ? (
              <Link
                ...
                className={`inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r ${colors.button} px-5 py-2.5 text-sm font-semibold text-white transition-opacity hover:opacity-90`}
              >
      ...
      <div className="space-y-3">
        {filteredItems.map((item) => {
          ...
          return (
            <div key={item.id} className="glass-card rounded-2xl p-5 transition-all hover:border-white/[0.15]">
              ...
              {showApplicationPipeline && ['applied', 'shortlisted', 'interview_scheduled', 'selected', 'rejected'].includes(item.status) ? (
                <div className="mt-5 border-t border-white/[0.06] pt-4 overflow-x-auto no-scrollbar">
                  <ApplicationPipeline
                    currentStatus={item.status as any}
                    timestamps={parseTimestamps(item.statusTimestamps)}
                    readonly={true}
                  />
                </div>
