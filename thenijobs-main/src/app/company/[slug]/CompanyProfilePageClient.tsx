...
export default function CompanyProfilePageClient({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    async function loadCompanyData() {
      try {
        ...
        // 1. Fetch company by direct document id or slug
        const directCompany = await getDoc(doc(db, 'companies', slug));
        if (directCompany.exists()) {
          const docData = { id: directCompany.id, ...directCompany.data() };
          setCompany(docData);
  ...
  const processedCompany = {
    ...company,
    coverUrl: company.coverUrl || company.coverImageUrl || '',
    coverImageUrl: company.coverImageUrl || company.coverUrl || '',
    gallery: company.gallery || company.galleryImages || company.galleryUrls || [],
    galleryImages: company.galleryImages || company.gallery || company.galleryUrls || [],
    posts: company.posts || [],
